...
export default defineComponent({
  ...
  setup() {
    ...
    const widthSize = ref(400);
    const heightSize = ref(160);
    const colorValue = ref("red");
    const parentStyleFun = (enlargeAmount, enlargeAmount1) => {
      heightSize.value += enlargeAmount;
      widthSize.value += enlargeAmount1;
    };
    return { post, postFontSize, widthSize, heightSize, colorValue };
  },
});
</script>
```
